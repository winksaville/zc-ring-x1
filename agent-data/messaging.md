...
The session behavior around the family's notification repo: what a session checks at acquaint,
and what it does with a request. The protocol itself is not here. The messages repo's `README.md`
is the authority on record shape, modes, and ownership, and this file defers to it on every point
it covers.
...
A project that is not a family member has no `[family]` table, and nothing in this file applies
to it.
...
- a record without `read:` is unread. Reading it is what adds the field, so the sender can see
  the message arrived.
...
reply cites that entry (wink, 2026-08-12). The reason is durability on both ends: a commit then
has an entry to reference, and the entry outlives the exchange, so the request's fate is readable
from this repo's records after the messages repo has moved on.
