...
- SPSC's one-accessor rule complicates
  ...
  is uniform: every context gets its own Sender, and the CAS
...
- Per-sender pending FIFOs compose with MPSC (each Sender
  owns its own, and per-producer order is all MPSC promises
  anyway, see
...
- `benches/` with `harness = false`: the pinned round-trip
  ...
  `cargo bench`, and they compile against dev-dependencies, so
...
- `tprobe` / `tp_runner`: path-dep to the sibling repo for
  now, and extract `tprobe` to its own repo when a second
