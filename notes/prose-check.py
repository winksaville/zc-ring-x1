...
# The characters are built from code points so this file passes its own check.
BANNED = "".join(chr(c) for c in (0x2014, 0x2013, 0x2026, 0x2192))
SPAN = re.compile(r"`[^`]*`")
...
def tracked():
    ...
    out = subprocess.run(
        ["jj", "file", "list"], check=True, capture_output=True, text=True
    ).stdout
    return [
        p
        for p in out.split("\n")
